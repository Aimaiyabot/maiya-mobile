  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await fetch("/api/maiyabot", {
        ...
        body: JSON.stringify({ messages: updatedMessages, name: userName, niche: userNiche }),
      });
      ...
      <div className="w-64 p-6 bg-white shadow-lg">
        <div className="text-center">
          ...
          {!editMode ? (
            <>
              ...
              <button className="text-xs text-pink-500 underline mt-2" onClick={() => setEditMode(true)}>Edit Profile</button>
            </>
          ) : (
            <div className="mt-4 space-y-2 text-left">
              ...
              <div className="flex gap-2 mt-1">
                <button className="text-xs bg-pink-500 text-white px-3 py-1 rounded" onClick={saveUserProfile}>Save</button>
                <button className="text-xs text-gray-500 underline" onClick={() => setEditMode(false)}>Cancel</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="mt-6">
          ...
          <button onClick={async () => {
            ...
            setMessages([]);
          }} className="text-sm text-pink-500 underline mt-4">Clear Chat</button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-between items-center px-6 py-8 relative">
        <div className="w-full max-w-4xl overflow-y-auto mb-6 pb-32" style={{ maxHeight: '70vh' }}>
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex items-start gap-2 mb-4 ${msg.role === 'user' ? 'justify-end flex-row-reverse' : 'justify-start'}`}>
              ...
              <div className={`rounded-xl px-4 py-3 max-w-[75%] whitespace-pre-line text-sm ${msg.role === 'user' ? 'bg-pink-100 text-pink-700 self-end' : `${getMoodColorClass(msg.content)} shadow-md self-start`}`}>
                <strong>{msg.role === 'user' ? 'You' : 'Maiya'}:</strong> {msg.content}
              </div>
            </div>
          ))}
        ...
        <div className="fixed bottom-6 w-full max-w-2xl px-6">
          <div className="flex bg-white border border-pink-300 rounded-full shadow-lg p-2 items-center relative">
            <button onClick={() => setShowEmojiPicker(prev => !prev)} className="text-xl px-3">😊</button>
            <textarea
              rows={1}
            ...
            <button
              ...
              className="mr-2 text-sm text-pink-500"
            ...
            <button
              onClick={sendMessage}
              className="bg-pink-500 hover:bg-pink-600 text-white rounded-full w-10 h-10 flex items-center justify-center"
            >
              ➤
            </button>
